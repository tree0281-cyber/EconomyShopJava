...
public class EconomyShopPlugin extends JavaPlugin implements Listener {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (name) {
            ...
            case "shopnpc": {
                ...
                if (sub.equals("spawn")) {
                    Villager v = pp.getWorld().spawn(pp.getLocation(), Villager.class, vil -> {
                        vil.setCustomName(ChatColor.GREEN + "상점 (우클릭)");
                        vil.setCustomNameVisible(true);
                        vil.getPersistentDataContainer().set(npcKey, PersistentDataType.BYTE, (byte) 1);
                    });
    ...
    public void onInteract(PlayerInteractEntityEvent e) {
        if (e.getRightClicked().getPersistentDataContainer().has(npcKey, PersistentDataType.BYTE)) {
            e.setCancelled(true);
